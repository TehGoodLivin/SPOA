...
do {
    ...
    switch -Wildcard ($menuMain) {
        ...
        "1*" {
            ...
            do {
                ...
                switch -Wildcard ($menuSub) {
                    ...
                    "5*" {
                        if ($menuSub.Contains("?")) { write-host "`nPROVIDES CAPABILITY TO ADD MULTIPLE SITE COLLECTION ADMINS." -ForegroundColor Green } else { 
        ...
        "2*" {
            if ($menuMain.Contains("?")) { 
                write-host "`nCONTAINS TOOLS SPECIFIC TO A GROUP." -ForegroundColor Green
            }
            ...
            do {
                ...
                switch -Wildcard ($menuSub) {
                    "1*" { 
                        if ($menuSub.Contains("?")) { write-host "`nPROVIDES CAPABILITY TO ADD EVERYONE CONTAINER TO GROUP." -ForegroundColor Green } else {
                            spoGroupAddEveryone -reportPath $setupReportPath -reportName "spoGroupAddEveryone_$((Get-Date).ToString("yyyyMMdd_HHmmss")).csv"
                        }
                    }
                }
            } until ($menuSub -eq "e")
        ...
        "3*" {
            ...
            do {
                ...
                switch -Wildcard ($menuSub) {
                    "1*" { 
                        if ($menuSub.Contains("?")) { write-host "`nPROVIDES CAPABILITY TO DELETE MULTIPLE USERS FROM SITE COLLECTION (SPO Account will remain)." -ForegroundColor Green } else {
                            spoUserDelete -reportPath $setupReportPath -reportName "spoUserDelete_$((Get-Date).ToString("yyyyMMdd_HHmmss")).csv"
                        }
                    }
                    "2*" { 
                        if ($menuSub.Contains("?")) { write-host "`nPROVIDES CAPABILITY TO DELETE ALL USER GROUPS FROM MULTIPLE USERS." -ForegroundColor Green } else {
